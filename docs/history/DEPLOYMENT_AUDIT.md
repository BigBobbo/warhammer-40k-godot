...
**Status**: **Implemented.** `ScoutPhase.gd` handles the full Scout Moves sub-phase between deployment and Turn 1. Units with Scout ability get a mini-movement phase with their designated range, must end >9" from enemy models. Player going first moves first. Phase auto-skips if no Scout units exist. Tests in `test_scout_moves.gd`. Additionally, `ScoutMovesPhase.gd` provides an alternative implementation with `BEGIN_SCOUT_MOVE`, `SET_SCOUT_MODEL_DEST`, `CONFIRM_SCOUT_MOVE`, `SKIP_SCOUT_UNIT`, and `END_SCOUT_MOVES` actions. Added `GameState.unit_has_scout()`, `GameState.get_scout_range()`, and `GameState.get_scout_units_for_player()` helpers. NetworkManager includes scout actions in `DETERMINISTIC_ACTIONS` for multiplayer sync.
...
| Update 6 | **Scout Moves (ScoutMovesPhase) merged.** Added `SCOUT_MOVES` phase to `GameState.Phase` enum alongside existing `SCOUT`. Created `ScoutMovesPhase.gd` with full action validation/processing. Added `get_scout_range()` alias. NetworkManager: scout actions in DETERMINISTIC_ACTIONS. TurnManager handles SCOUT_MOVES completion. |
