...
| **A4b** (COUNTEROFFENSIVE 15.12 target + timing + 19.01) | Reported from a live game: the Counter-Offensive dialog listed an **attached CHARACTER as its own target row** (Blade Champion leading Custodian Guard got a "Fight Next (2 CP)" row next to theirs) — 19.01 makes an attached unit "a single unit for all rules purposes". Root cause: `StratagemManager.get_counter_offensive_eligible_units` walked raw unit dicts and still applied the **10e** target condition ("within Engagement Range … has not already been selected to fight"). Although `_resolve_core_id` had already routed CP/usage/effects to `counteroffensive_11e`, the *target* and *timing* halves of 15.12 were never migrated. Now: (1) each Attached unit folds onto its bodyguard (one row, named "Custodian Guard + Blade Champion"; fought/battle-shock/alive asked of the whole group; 19.05 hands the character his own row back once the bodyguard is wiped); (2) the target list comes from the live `FightSequencer.eligible_units` — 12.04's *eligible to fight* (engaged, **or engaged at the start of the step, or charged this turn**) rather than a bare Engagement-Range test; (3) the window is offered **only to the non-active player** ("Fight step of your **opponent's** Fight phase"), where before the active player was offered it whenever a defender swung back; (4) the dialog prints the 11e effect (Fights First until end of phase + must be your next selection) instead of the retired 10e "your unit fights next". | Windowed scenario `counter_offensive_attached_char_11e` (41/0): P2 Warboss resolves its attacks → the window opens for P1 with rows read off the real Labels = `Custodian Guard + Blade Champion\|Telemon Heavy Dreadnought` (no `CO_U_BLADE_CHAMPION_A` node), clicking it spends 2 CP (4→2), sets `active_fighter_id=U_CUSTODIAN_GUARD_B` and stamps `flags.fights_first`. Before/after captured on the same fixture: at HEAD the same path rendered **three** rows — `Blade Champion\|Custodian Guard\|Telemon Heavy Dreadnought`. Headless: 7 new cases in `tests/unit/test_counter_offensive.gd`. |
