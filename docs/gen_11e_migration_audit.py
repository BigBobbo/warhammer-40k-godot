...
("23.01/02/03","Aircraft reserve cycle","10e zoom/hover movement","AIRCRAFT must start in reserves; ingress-only; return to reserves each opp turn","done","GameState.unit_must_start_in_reserves + return_aircraft_to_reserves; DeploymentPhase rejects on-board AIRCRAFT; TurnManager MORALE hook runs the end-of-turn return. Edition+keyword gated. Player-reachable via the Orks Wazbom Blastajet (AIRCRAFT) — windowed scenario drives the real Wazbom: deploy-on-board rejected, reserves placement, end-of-turn return (ISS-074)"),
...
("24.20","Infiltrators","deploy >9\" from enemy DZ & models","deploy >8\" HORIZONTALLY from enemy DZ & all enemy units","done","DeploymentPhase._validate_infiltrators_position uses infiltrate_min = 8.0 at ed≥11 (else 9.0) across all 6 sites (ISS-068; windowed scenario drives the live validator for the Orks Kommandos — 8.2\" accepted at ed11, rejected at ed10)"),
...
("HIGH","12.02/03","11e pile-in never reaches the live Fight phase","PileInMove (5\" target select, base-contact lock) is implemented & unit-tested but FightPhase._validate_pile_in / _process_pile_in run hardcoded legacy 3\"/closest-model logic with no edition branch.","FightPhase.gd:639,1267","FIXED (ISS-066): _validate_pile_in branches to _validate_pile_in_11e at edition≥11. Validated headless against the real FightPhase validators + a windowed scenario that drives the live FightPhase: the same pile-in move is rejected with the 12.03 reason at edition 11 vs the legacy 'toward closest enemy' message at edition 10."),
...
("LOW","24.35","Super-Heavy Walker MOBILE-grant + D6 battle-shock gamble not driven","The ≤4\" terrain traversal works; the optional 'grant all models MOBILE then roll D6, 1 = battle-shocked' is not applied by any action.","MovementPhase.gd + TerrainManager.gd","FIXED (ISS-073): begin handlers record shw_mobile from payload.shw_mobile_gamble (ed≥11 + SUPER-HEAVY WALKER); MOBILE passed to the 13.06 traversal; seeded D6 at move-confirm sets battle_shocked on a 1. Second-pass fix: the phase logic had no UI — MovementController now shows a 'Risk MOBILE' toggle for a SUPER-HEAVY WALKER at ed≥11 and folds it into the BEGIN payload. 13-assertion headless test + a windowed scenario that real-clicks the toggle + Confirm Movement Mode (a SHW fixture is generated since the default armies have none)."),
("LOW","23.01/02","Aircraft reserve cycle absent (was mislabelled a no-datasheet deferral)","No forced-reserves at deployment and no return-to-reserves cycle. Originally called 'no aircraft datasheets exist' — WRONG: the Orks army contains the Wazbom Blastajet (AIRCRAFT), so it is player-reachable.","(was unimplemented)","FIXED (ISS-074): GameState forced-reserves + return-to-reserves helpers; DeploymentPhase rejects on-board AIRCRAFT; TurnManager MORALE hook runs the end-of-turn return. 23-assertion headless test + a windowed scenario driving the real Orks Wazbom Blastajet (deploy-on-board rejected, reserves placement, end-of-turn return)."),
