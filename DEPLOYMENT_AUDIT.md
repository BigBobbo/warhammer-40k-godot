...
**Status**: **Implemented.** Added `SCOUT_MOVES` phase to the `GameState.Phase` enum between `DEPLOYMENT` and `COMMAND`. Created `ScoutMovesPhase.gd` (`40k/phases/ScoutMovesPhase.gd`) extending `BasePhase` with full action validation and processing. The phase supports: `BEGIN_SCOUT_MOVE`, `SET_SCOUT_MODEL_DEST`, `CONFIRM_SCOUT_MOVE`, `SKIP_SCOUT_UNIT`, and `END_SCOUT_MOVES` actions. Validation enforces: unit must have Scout ability, must be deployed, move distance <= Scout range, destination >9" edge-to-edge from all enemy models, destination on board. Player ordering follows rules — first turn player scouts first. Phase auto-skips if no units have Scout. Added `GameState.unit_has_scout()`, `GameState.get_scout_range()`, and `GameState.get_scout_units_for_player()` helpers. PhaseManager registers `ScoutMovesPhase` and transitions DEPLOYMENT → SCOUT_MOVES → COMMAND. GameManager routes all scout actions. NetworkManager includes scout actions in `DETERMINISTIC_ACTIONS` for multiplayer sync. TurnManager resets active player to Player 1 after scout moves complete. Main.gd UI shows scout-eligible units with `[Scout X"]` tags, provides click-to-move-unit interaction, confirm/skip buttons, and board click handling for group movement. Army data updated: Space Marines Infiltrator Squad and Ork Kommandos now have `Scout 6"` ability.
...
| Update 5 | **Scout Moves marked DONE.** Added `SCOUT_MOVES` phase to `GameState.Phase` enum. Created `ScoutMovesPhase.gd` with full action validation/processing (BEGIN_SCOUT_MOVE, SET_SCOUT_MODEL_DEST, CONFIRM_SCOUT_MOVE, SKIP_SCOUT_UNIT, END_SCOUT_MOVES). Validates: Scout ability, move range, >9" edge-to-edge from enemies, on-board. Player order: first-turn player scouts first. Auto-skips if no scouts. Added `unit_has_scout()`, `get_scout_range()`, `get_scout_units_for_player()` to GameState. PhaseManager: DEPLOYMENT → SCOUT_MOVES → COMMAND. GameManager routes scout actions. NetworkManager: scout actions in DETERMINISTIC_ACTIONS. TurnManager resets active player after scouts. Main.gd: scout unit list with [Scout X"] tags, click-to-move interaction, confirm/skip buttons. Army data: Space Marines Infiltrator Squad and Ork Kommandos now have Scout 6". |
