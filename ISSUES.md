...
- **Windowed:** `tests/scenarios/sp/iss066_pile_in_consolidate_11e.json` (15/15) enters the live FIGHT phase at edition 11, selects a fighter via the real `SELECT_FIGHTER` (11e FightSequencer active), and drives the live `validate_action(PILE_IN)`: an empty pile-in completes the mandatory step, a move that does not end closer is rejected with the **12.03** reason, and the **same move at edition 10 is rejected with the legacy "toward closest enemy" message** — proving the validator branches to the 11e template by edition in the running game.
