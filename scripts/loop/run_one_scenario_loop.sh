#!/bin/bash
# Per-scenario visual-regression loop driver (Phase 1 + stub critic).
#
# Runs a single scenario windowed with per-step screenshots enabled,
# then invokes the stub critic to validate the I/O contract. The real
# critic + fixer are Agent subagents invoked by the cloud Claude session
# that calls this script — they live in critic_prompt.md / fixer_prompt.md.
#
# Usage:
#   bash scripts/loop/run_one_scenario_loop.sh <scenario-json-path>
#
# Exit codes:
#   0   scenario ran cleanly and critic produced critique.json
#   1   scenario or critic stub failed
...
rm -f "$RESULTS_DIR/critique.json" 2>/dev/null || true
...
# 4. Invoke the stub critic (real critic is an Agent in the cloud session).
echo ""
