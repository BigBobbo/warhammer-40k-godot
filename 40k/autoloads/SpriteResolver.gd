...
#
# Bundled sprites: the game ships top-down unit art in res://assets/unit_sprites/
# (generated by tools/sprites/generate_unit_sprites.py). At each priority level
# a user://sprites/ file overrides the bundled one, so players can still re-skin
# any unit by dropping a PNG with the same name into user://sprites/.
#
...
# Bundled (shipped-with-the-game) top-down unit art. Checked per priority level
# after user:// overrides. Resolved via ResourceLoader so it works both in the
# editor and in exported builds (where imported PNGs are remapped).
const BUNDLED_SPRITES_DIR := "res://assets/unit_sprites/"
...
func resolve_sprite(unit_name: String, faction_name: String, unit_type: String) -> Texture2D:
	...
	if clean_faction != "" and clean_type != "":
		keys.append(clean_faction + "_" + clean_type)
	if clean_type != "":
		keys.append(clean_type)

	for key in keys:
		if _available_sprites.has(key):
			return _load_texture(_available_sprites[key])
		var bundled := _load_bundled_texture(key)
		if bundled != null:
			return bundled
...
func _load_bundled_texture(key: String) -> Texture2D:
	# Load a shipped sprite from res://assets/unit_sprites/<key>.png, or null.
	var path := BUNDLED_SPRITES_DIR + key + ".png"
	...
	DebugLogger.info("[SpriteResolver] Loaded bundled sprite: %s" % path)
	return texture
