...
func load_state(data: Dictionary) -> void:
	...
	# Rebuild the faction stratagem DEFINITIONS for the loaded armies. Only the
	# per-player id lists are serialized; on a fresh process the boot-time
	# default armies may differ from the saved ones (e.g. a Lions of the
	# Emperor save loaded after boot registered Shield Host), which left the
	# restored ids without definitions ("Unknown stratagem" on use). The
	# reload reads GameState.state.factions — already restored at this point —
	# and repopulates the id lists identically.
	load_all_faction_stratagems()
