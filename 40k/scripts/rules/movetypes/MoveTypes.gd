...
static func _ensure() -> void:
	if _registry.is_empty():
		for mt in [RemainStationaryMove.new(), NormalMove.new(), AdvanceMove.new(), FallBackMove.new(), IngressMove.new(), SurgeMove.new(), DisembarkMove.new(), EmergencyDisembarkMove.new()]:
			_registry[mt.id] = mt
