...
func _handle_mouse_release(global_pos: Vector2) -> void:
	...
	var charge_controls = get_node_or_null("/root/Main/HUD_Bottom/HBoxContainer/ChargeControls")
	if charge_controls and is_instance_valid(charge_controls):
		charge_controls.queue_free()
	
	var charge_scroll = get_node_or_null("/root/Main/HUD_Right/VBoxContainer/ChargeScrollContainer")
	if charge_scroll and is_instance_valid(charge_scroll):
		charge_scroll.queue_free()
...
func _refresh_ui() -> void:
	...
	if not is_instance_valid(unit_selector):
		return
	unit_selector.clear()
	var current_player = current_phase.get_current_player() if current_phase else GameState.get_active_player()
	var units = current_phase.get_units_for_player(current_player) if current_phase else GameState.get_units_for_player(current_player)
	
	print("ChargeController: Refreshing UI for player ", current_player, " with ", units.size(), " units")
	
	var can_charge_count = 0
	for unit_id in units:
		var unit = units[unit_id]
		print("  Checking unit ", unit_id, " status=", unit.get("status", -1), " flags=", unit.get("flags", {}))
		if _can_unit_charge(unit):
			...
			print("    Unit ", unit_id, " can charge!")
		else:
			print("    Unit ", unit_id, " cannot charge")
	
	print("ChargeController: Found ", can_charge_count, " units that can charge")
...
func _start_model_drag(model: Dictionary, world_pos: Vector2) -> void:
	...
	circle.color = Color(0, 1, 0, 0.5)  # Semi-transparent green
	...
	line.add_point(_get_model_position(model))
...
func _update_model_drag(world_pos: Vector2) -> void:
	if not ghost_visual or not dragging_model:
		return
	...
	if ghost_visual.get_child_count() > 0:
		var circle = ghost_visual.get_child(0)
		if circle.has_method("set_color"):
			circle.color = Color.GREEN if is_valid else Color.RED
...
func _validate_charge_position(model: Dictionary, new_pos: Vector2) -> bool:
	...
	# Check 2: Must end within engagement range of at least one target
	var model_radius = Measurement.base_radius_px(model.get("base_mm", 32))
	...
	for target_id in selected_targets:
		...
		for target_model in target.get("models", []):
			...
			var target_radius = Measurement.base_radius_px(target_model.get("base_mm", 32))
			var edge_distance = new_pos.distance_to(target_pos) - model_radius - target_radius
			
			if edge_distance <= engagement_px:
				in_engagement = true
				break
	...
	if not in_engagement:
		print("Not in engagement range of any target")
		return false
...
func _on_confirm_charge_moves() -> void:
	...
	print("Sending charge movement action: ", action)
	print("DEBUG: per_model_paths = ", per_model_paths)
	print("DEBUG: moved_models = ", moved_models)
	print("DEBUG: active_unit_id = ", active_unit_id)
