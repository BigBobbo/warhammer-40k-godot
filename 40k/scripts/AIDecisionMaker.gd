...
static func _select_movement_action(snapshot: Dictionary, available_actions: Array, player: int) -> Dictionary:
	...
	# Pick first movable unit
	var unit_id = movable_units.keys()[0]
	var move_types = movable_units[unit_id]
	var unit = snapshot.get("units", {}).get(unit_id, {})
	var unit_name = unit.get("meta", {}).get("name", unit_id)

	# Decide movement type
	# If engaged, must fall back or remain stationary
	...
	# For now, use REMAIN_STATIONARY as the simplest safe option
	# This avoids complex position calculation issues with movement validation
	...
	# Fallback: normal move but remain stationary if available
	return {"type": "END_MOVEMENT", "_ai_description": "End Movement Phase"}
