...
# The big parchment/gold D6 face used here now lives in its own script so the
# Command-phase Battle-shock roll call can roll IDENTICAL dice (see
# scripts/DiceFaceVisual.gd). Aliased to the old inner-class name so every
# `DiceFace.new()` / `DiceFace.Highlight.*` call site below reads unchanged.
...
# -----------------------------------------------------------------------------
# NOTE: the DiceFace class that used to live at the bottom of this file is now
# scripts/DiceFaceVisual.gd (see the `const DiceFace` alias at the top).
# -----------------------------------------------------------------------------
