...
SECTIONS = [
    ...
    },
    {
        ...
        "shot": None,
        "states": [{
            "name": "Fight board — Fight step (12.04)",
            "when": "The alternating fighter-selection step; no global step running.",
            ...
            "notes": ["Bumpers cycle the panel's action buttons; A commits the focused one; D-pad navigates the panel.",
                      ...
                      "Start = End Fight Phase (the only Fight-phase step whose end actually ends the phase).",
                      "GAP (not pad-specific): per-model weapon choice, splitting attacks across targets, and SELECT_MELEE_WEAPON have no human UI."],
        }, {
            "name": "Global Pile In step (12.02)",
            "when": "The PILE IN STEP section is live on the right panel (the phase opens here).",
            "hint_set": "HINTS_FIGHT_PILE_IN",
            "rp": RP_FIGHT,
            "notes": ["Start presses the section's own 'End Pile In (Player N)' button — the same button the mouse clicks — so it ends YOUR pile-in half, not the phase.",
                      "The top-right phase-action button drops its '[☰] ' prefix while this step owns Start, so only one thing on screen claims ☰.",
                      "Same chip from panel focus: the ☰ label is appended to HINTS_FOCUS while a step section is live, because focus never swallows Start.",
                      "While a chosen unit's pile-in move is on the board the section is greyed out and Start goes back to the phase action — the PileInDialog owns ☰ (Confirm Move) via its own on-dialog hint row."],
        }, {
            "name": "Global Consolidate step (12.07)",
            "when": "The CONSOLIDATE STEP section is live on the right panel (after all fighting).",
            "hint_set": "HINTS_FIGHT_CONSOLIDATE",
            "rp": RP_FIGHT,
            "notes": ["Start presses the section's own 'End Consolidation (Player N)' button — ends your consolidation half, not the phase.",
                      "Mirrors the Pile In step in every respect; the phase completes once both halves are done."],
        }],
    },
    {
...
GAPS = [
    ...
    ("warning", "Datasheet: pad-B doesn't close it, and the pad can't scroll it",
     "Only Y toggles it closed on pad (Esc on keyboard). The card now sizes to its content and scrolls when it overflows, but only the mouse wheel drives that scroll — the pad has no binding for it.",
     "DatasheetModal.gd:142-181 · Main.gd:5829-5832"),
