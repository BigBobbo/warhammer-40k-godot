...
- [x] Implement TITANIC unit deployment skip — When a player deploys a TITANIC unit, they skip their next deployment turn. Detect TITANIC keyword in `TurnManager.check_deployment_alternation()` and skip the deploying player's next turn (DEPLOY-RULES-3)
...
- [x] Enforce Extra Attacks number cannot be modified — Balance Dataslate: "number of attacks made with an Extra Attacks weapon cannot be modified by other rules, unless that weapon's name is explicitly specified." Add validation in RulesEngine.gd attack count calculation. (SHOOT-4)
