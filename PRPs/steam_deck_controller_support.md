...
**Status:** IN PROGRESS — M0 shipped 2026-07-12 (v0.25.0, PR #571); M1+M2 shipped 2026-07-12 (v0.31.0/v0.32.0, PR #576); M3 shipped 2026-07-13 (v0.33.0; gates `pad_m3_carry_move` + `pad_m3_carry_deploy` PASS — stick-carried models through the real drag/stage/coherency pipeline in Movement and Deployment). M4 started 2026-07-19 (v0.65.0): Movement move-mode **action menu** (`PadActionBar` autoload — A on a unit offers Move / Advance / Fall Back / Stay Still / special actions instead of silently beginning a Normal Move; Move/Fall Back auto-flow into the carry), eligibility-aware bumper cycling in Movement (completed units skipped, via `MovementController.pad_can_cycle_to`), and carry-cancel no longer leaves a 0" staged move (gate `pad_m4_move_action_menu` PASS; full pad suite + `51_confirm_movement_mode` KBM regression 10/10). Also in M4 (v0.65.1): dialog-focus **watchdog** in `InputDeviceManager` — fixes the live-reported Deck stall where the first-turn RollOffDialog popped before the session's first pad input (nothing focused → A dead) and refocuses dialogs whose internal buttons swap mid-flow (Roll → Continue); gate `pad_m4_rolloff` PASS. Also in M4 (v0.66.0): main-menu **controller status indicator** (top-right; not detected / connected / ACTIVE) — field diagnosis for the Deck keyboard-emulation-template failure mode ("D-pad pans the camera, nothing selects" = the game is receiving arrow keys, not joypad input; no in-game pad code can help until the Steam Input layout is set to Gamepad); gate `pad_m4_controller_indicator` PASS. Next in M4: charge/pile-in carry scenarios, Command/Scoring sweep, reactive-dialog pass, Tools menu on View, OSK.
